...
def check_spatial_constraints(blocks_state, spatial_goals, tolerance=0.03):
    """
    ...
    Args:
        ...
        tolerance: Position tolerance in meters (default 3cm)
    
    Returns:
        Tuple of (all_satisfied, violations)
    """
    violations = []
    ...
    for block_name, target_pos in spatial_goals.items():
        ...
        if dx > tolerance or dy > tolerance:
            violations.append(
                f"{block_name}: at ({current_pos[0]:.3f}, {current_pos[1]:.3f}), "
                f"should be at ({target_pos[0]:.3f}, {target_pos[1]:.3f}), "
...
def goal_achieved_with_spatial(current_state, goal_predicates, blocks_state):
    ...
    Returns:
        bool: True if goal fully achieved
    """
    ...
    if "spatial" in goal_predicates:
        ...
        if not spatial_satisfied:
            print("[SPATIAL] Constraints not satisfied:")
            for v in violations:
                print(f"  {v}")
            return False
        else:
            print("[SPATIAL] ✓ All spatial constraints satisfied")
