...
# CONSTANTS
# ============================================================
BLOCK_SIZE = 0.04  # 4cm cubic block

# XY alignment tolerance for stacking
...
# Blocks below this height are on the table
...
def get_block_positions(blocks_state):
    ...
    Args:
        blocks_state: dict mapping block names to Genesis entities
            Example: {"r": cubeR, "g": cubeG}
    
    Returns:
        dict: block_name -> position as np.array([x, y, z])
...
def get_gripper_state(robot):
    ...
    Returns:
        tuple: (is_holding, object_name)
            is_holding (bool): True if gripper has something
            object_name (str or None): Name of held object
    """
    ...
    if held_obj is not None:
        ...
        if object_name is None:
            # try to find name by searching blocks_state
            object_name = str(held_obj)
...
def compute_predicates(robot, blocks_state, scene=None,
    ...
    This is the core function that bridges geometric state to symbolic reasoning.
    
    Args:
        robot: Robot entity
        blocks_state: dict of block_name -> Genesis entity
        scene: Genesis scene (optional, for future use)
        xy_threshold: Horizontal alignment tolerance (meters)
        z_min: Min vertical separation for ON predicate
        z_max: Max vertical separation for ON predicate
    
    Returns:
        dict with keys:
            "on": [(block_a, block_b), ...] - a is on top of b
            "ontable": [block_name, ...] - blocks on table
            "clear": [block_name, ...] - nothing on top
            "holding": [block_name] - block in gripper (max 1)
            "handempty": [True] or [] - gripper empty
    
    Example:
        Red on green, green on table, blue on table:
        {
            "on": [("r", "g")],
            "ontable": ["g", "b"],
            "clear": ["r", "b"],
    ...
    # For each block, check if on top of another block
    for name_a, pos_a in block_positions.items():
        # Skip blocks currently being held (not on anything)
        ...
        for name_b, pos_b in block_positions.items():
            if name_a == name_b:
                continue  # Block cant be on itself
            
            # Skip if block_b is being held
        ...
        # If not on block, must be on the table
...
"""
    Generate PDDL problem file for Pyperplan
    
    Args:
        predicates: Current state predicates (from compute_predicates)
        goal_predicates: Desired goal state
        filename: Output file path
        domain_name: PDDL domain name (must match domain file)
        problem_name: Problem instance name
    
    Returns:
        str: Absolute path to generated file
...
def goal_achieved(current_predicates, goal_predicates, strict=True):
    ...
    Args:
        current_predicates: Current symbolic state
        goal_predicates: Target state
        strict: If True, exact match. If False, goal can be subset of current
    
    Returns:
        bool: True if goal is achieved
    
    Example:
        goal = {"on": [("r", "g")], "ontable": ["g"]}
        current = {"on": [("r", "g")], "ontable": ["g", "b"]}
        
        goal_achieved(current, goal, strict=False) → True
        goal_achieved(current, goal, strict=True) → False (extra ontable)
    """
    for predicate_type, goal_values in goal_predicates.items():
        if not goal_values:
            continue
        
        # Check if this predicate
        ...
        # In strict mode,  check no extra predicates
...
def check_tower_stable(blocks_state, tower_blocks, 
    ...
    """
    Check if a tower of blocks is physically stable
    
    Args:
        blocks_state: dict of block_name -> Genesis entity
        tower_blocks: list of block names, bottom to top
            Example: ["b", "g", "r"] for blue-green-red tower
        xy_threshold: Max horizontal misalignment (meters)
        z_tolerance: Vertical position variation tolerance
    
    Returns:
        bool: True if tower is stable
    """
    ...
    # Check alignment of each block above
...
def get_tower_height(block_name, predicates):
    """
    Count blocks stacked above a given block (including the block itself)
    
    Example:
        on(r, g), on(g, b)
        get_tower_height("b") → 3 (b, g, r)
        get_tower_height("g") → 2 (g, r)
    """
    height = 1
    current = block_name
...
def get_all_towers(predicates):
    """
    Identify all distinct towers in the scene
...
def visualize_predicates(preds, title="Current Symbolic State"):
    """Print predicates in readable format"""
    print(f"\n{'=' * 50}")
    print(f"{title:^50}")
    print('=' * 50)
...
def compare_states(current_preds, goal_preds):
    """Compare current state with goal and show differences"""
...
def calculate_grid_positions(center, spacing, grid_shape):
    """
    Calculate positions for a rectangular grid
...
def calculate_2x2_grid_positions(center, spacing):
    ...
    Returns:
        List of 4 (x, y) positions: [bottom-left, top-left, bottom-right, top-right]
    """
    half = spacing / 2
    return [
...
def check_spatial_constraints(blocks_state, spatial_goals, tolerance=0.020):
    ...
    Args:
        blocks_state: dict of block_name -> block object
        spatial_goals: dict of block_name -> target (x, y) position
        tolerance: Position tolerance in meters (default 20mm)
    
    Returns:
        tuple: (all_satisfied: bool, violations: list)
    """
    violations = []
...
def goal_achieved_with_spatial(current_state, goal_predicates, blocks_state):
    ...
    Args:
        current_state: Current symbolic predicates
